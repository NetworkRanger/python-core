...
class GTKapp(object):
    def __init__(self):
        top = gtk.Window(gtk.WINOW_TOPLEVEL)
        top.connect('delete_event', gtk.main_quit)
        top.connect('destroy', gtk.main_quit)
        box = gtk.VBox(False, 0)
        lb = gtk.Label('Animals (in pairs; min: pair, max: doze)')
        box.pack_start(lb)
        ...
        sty = qb.get_style()
        for st in (gtk.STATE_NORMAL, gtk.STATE_PRELIGHT, gtk.STATE_ACTIVE): sty.bg[st] = red
        qb.set_style(sty)
        ql = qb.child

...
if not APIs:
    raise NotImplementedError('No Twitter API found; install one & add to CMDs!')
...
class Twitter(object):
    ...
    def search(self, q):
        ...
        if api == 'twython':
            ...
            return (ResultsWrapper(tweet) for tweet in self._get_meth('search')(q=q))

    def verify_credentials(self):
        return ResultsWrapper(self._get_meth('verify_credentials'))

    def user_timeline(self):
        return (ResultsWrapper(tweet) for tweet in self._get_meth('user_timeline')())

    def update_status(self, s):
        return ResultsWrapper(self._get_meth('update_status')(status=s))
...
class ResultsWrapper(object):
    ...
    def __getattr__(self, attr):
        if hasattr(self.obj, attr):
            ...
            raise AttributeError('%r has not attribute %r' % (self.obj, attr))
...
def _demo_update_status():
    for api in APIs:
        t = Twitter(api)
        res = t.update_status('Test tweet posted to Twitter using %s' % api.title())

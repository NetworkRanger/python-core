...
class PlusService(object):
    def __init__(self):
        self.service = build('plus', 'v1', developerKey=API_KEY)

    def get_posts(self, q, oldset, maxp=MAX_TOT):
        posts = []
...
class PlusPost(object):
    def __init__(self, record):
        ...
        self.post = scrub(record['object'].get('originalContent', ''))
        ...
        actor = record['actor']
        self.author = '%s (%s)' % (actor['displayName'], actor['id'])
        obj = record['object']
        cols = ('replies', 'plusoners', 'resharers')
        self.chatter = sum(obj[col]['totalItems'] for col in cols)

def top_posts(query, maxp=MAX_DEF, ndays=7):
    print '''
*** Searching for the top %d posts matching %r over the past %d days...''' % (maxp, query, ndays)
    oldest = (dt.datetime.now()-dt.timedelta(ndays)).isoformat()
    posts = service.get_posts(query, oldest, maxp)
    if not posts:
        print '*** no results found... try again ***'
        return
    sorted_posts = sorted(posts, reverse=True, key=lambda post: post.chatter)
    for i, post in enumerate(sorted_posts):
        print '\n%d)' % (i+1)
        print 'Form:', post.author
        print 'Date:', post.when.ctime()
        print 'Chatter score:', post.chatter
        print 'Post:', post.post if len(post.post) > len(post.title) else post .title
...
def find_user():
    ...
    print 'Abount:', user.get('abountMe', '')
...
def _main():
    ...
    prompt.insert(0, '%s\n%s%s' % (HR, 'Google + Command-Line Tool v0.3'.center(WIDTH), HR))
    prompt.append('\nEnter choice [QUIT]: ')
    prompt = '\n'.join(prompt)
    while True:
